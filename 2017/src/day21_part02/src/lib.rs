...
pub fn do_puzzle(input: &str, iteration_count: u32) -> usize {
...
#[derive(Debug)]
struct Rule {
    predicate: HashSet<Grid>,
    output: Grid,
}

impl Rule {
    fn matches(&self, grid: &Grid) -> bool {
        self.predicate.contains(grid)
        /*
        // check given grid
...
impl<'a> From<&'a str> for Rule {
    fn from(s: &str) -> Self {
        ...
        let predicate = predicate.generate_permutations().into_iter().collect();
...
impl Grid {
    ...
    fn generate_permutations(&self) -> Vec<Self> {
        vec![
            self.clone(),
            self.rotate(),
            self.rotate().rotate(),

...
impl<'a> Solver<'a> {
    fn walk(&self, used_pieces: &HashSet<usize>, previous: usize) -> usize {
        ...
        for (i, &(left, right)) in self.nodes.iter().enumerate() {
            if (left == previous || right == previous) && !used_pieces.contains(&i) {
                ...
                let path_len = self.walk(&used_pieces, left + right - previous);
                used_pieces.remove(&i);
                path.push(path_len + left + right);
            }
        }

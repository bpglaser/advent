...
impl Grid {
    fn tick(&mut self) {
        ...
        match *self.get(&carrier_position) {
            Node::Clean => {
                self.carrier.left();
                *self.get_mut(&carrier_position) = Node::Weakened;
            }
            Node::Weakened => {
            ...
            Node::Infected => {
                self.carrier.right();
                *self.get_mut(&carrier_position) = Node::Flagged;
            }
            Node::Flagged => {
                self.carrier.right();
                self.carrier.right();
...
fn test_example() {
    ...
    assert_eq!(26, grid.infection_count);
    for _ in 100..10000000 {
        grid.tick();
    }
    assert_eq!(2511944, grid.infection_count);
}

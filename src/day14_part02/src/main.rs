...
fn main() {
    let (salt, hash_count) = get_params();
    ...
    while pending_keys.iter().take_while(|k| k.finalization.is_some()).count() < 64 {
        ...
        let hash = multi_hash(&source, hash_count);
...
fn multi_hash(source: &str, hash_count: usize) -> String {
    let mut hash = md5_hash(source);
...
fn get_params() -> (String, usize) {
    (args().nth(1).unwrap(), args().nth(2).unwrap().parse().unwrap())
}

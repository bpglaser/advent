...
fn main() {
    ...
    let mut groups = read_input(&path);

    while has_remaining_units(&groups) {
        step(&mut groups);
    }
...
fn has_remaining_units(groups: &[Group]) -> bool {
    ...
    has_immune_system && has_infection
}

fn step(groups: &[Group]) {
...
impl Group {
    ...
    fn attack(&self, target: &Self) {
        ...
        target
            .units
            .set(target.units.get().checked_sub(dmg).unwrap_or_default());
    }

...
def main():
    ...
    biggestsleeper = (None, 0, 0)
    for num, freq in freqs.items():
        hour, cnt = max(enumerate(freq), key=lambda entry: entry[1])
        if cnt >= biggestsleeper[2]:
            biggestsleeper = (num, hour, cnt)

    print(biggestsleeper[0], biggestsleeper[1])
    print(biggestsleeper[0] * biggestsleeper[1])

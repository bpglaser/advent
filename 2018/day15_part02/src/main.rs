...
fn solve(input: &str) -> i32 {
    ...
    let mut orig_num_elves = 0;
    for row in &grid {
        for tile in row {
            match tile {
                Tile::Elf(_) => orig_num_elves += 1,
                _ => {}
            }
        }
    }
    ...
    for ap in 1.. {
        ...
        let mut rounds = 0;
        change_elf_ap(&mut grid, ap);

        loop {
            match do_round(&mut grid, orig_num_elves) {
                RoundResult::ElvesWin => break,
                RoundResult::ElvesWinPerfectly => {
                    println!("Final state with {} ap after {} rounds:", ap, rounds);
                    render(&grid);
                    println!();

                    return score(rounds, &grid);
                }
                RoundResult::ElvesWinPerfectlyOBO => {
                    println!("Final state with {} ap after {} rounds:", ap, rounds);
                    render(&grid);
                    println!();

                    return score(rounds + 1, &grid);
                }
                RoundResult::GoblinsWin => break,
                RoundResult::Unfinished => rounds += 1,
            }
...
fn change_elf_ap(grid: &mut Grid, ap: i32) {
    for row in grid {
        for tile in row {
            match tile {
                Tile::Elf(meta) => meta.attack_power = ap,
                _ => {}
            }
        }
    }
...
fn do_round(grid: &mut Grid, orig_num_elves: usize) -> RoundResult {
    let mut todo = vec![];
    for y in 0..grid.len() {
    ...
    for (i, (x, y)) in todo.into_iter().enumerate() {
        ...
        if goblins == 0 {
            if elves == orig_num_elves {
                if i == num_todo - 1 {
                    return RoundResult::ElvesWinPerfectlyOBO;
                } else {
                    return RoundResult::ElvesWinPerfectly;
                }
            } else {
                return RoundResult::ElvesWin;
            }
        } else {
            if elves == 0 {
                return RoundResult::GoblinsWin;
            }
        }
    }
...
mod tests {
    ...
    fn test_given0() {
        ...
        assert_eq!(4988, solve(&input));
    }

    #[test]
    fn test_given1() {
        let input = include_str!("../given1.txt");
        assert_eq!(31284, solve(&input));
    }

    #[test]
    fn test_given2() {
        let input = include_str!("../given2.txt");
        assert_eq!(3478, solve(&input));
    }

    #[test]
    fn test_given3() {
        let input = include_str!("../given3.txt");
        assert_eq!(6474, solve(&input));
    }

    #[test]
    fn test_given4() {
        let input = include_str!("../given4.txt");
        assert_eq!(1140, solve(&input));
    }
}

...
let rec search grid seen frontier =
    ...
    | None -> grid
    | Some((pos, result), frontier) ->
        let grid =
            if isAtOxygenSystem result then Map.add pos Goal grid
            else Map.add pos Space grid
...
let findSolution state =
    ...
    let goal = grid |> Seq.find (fun entry -> entry.Value = Goal)
    let goal = goal.Key
        ...
        seq {
            ...
            | Some((pos, depth), frontier) ->
                ...
                let frontier =
                    ...
                    |> getPosNeighbors
                    |> Seq.filter (fun pos ->
                        Set.contains pos seen
                        |> not
                        && Map.containsKey pos grid)
                    |> Seq.fold (fun q pos -> Queue.enqueue (pos, depth + 1) q) frontier
                yield! bfs seen frontier
        }
    bfs Set.empty (Queue.ofSeq [ (goal, 0) ])
    |> Seq.map snd
    |> Seq.max
